...
void removeTheLoop(Node *head) {
    ...
    // find it there is a loop for sure
    ...
    while(slow && fast && fast->next) {
        ...
        // check if we arrive at same node
        if(slow == fast) break;
    }
    
    // return if no loop exists
    if(!slow || !fast || !fast->next) return;
    
    // remove loop
    slow = head;
    
    // get at the the intersection point
    while(slow->next != fast->next) {
        slow = slow->next;
        fast = fast->next;
    }
    
    // remove the looping edge
    fast->next = NULL;
    return;
}