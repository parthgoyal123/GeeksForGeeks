...
void getVal(ll int *arr, ll int *brr, int n) {
    ...
    loop(i,0,n) {
        ...
        loopr(j,p-1,0) {
            ll digits = ((ll) sum_of_digits[j])%md;
            ll num = ( (ll) pow(10, power) )%md;

            sum = (sum%md + (digits*num)%md)%md;

            ll num1 = ((ll) n*(arr[i]%10))%md;
            num = ((ll) pow(10, power+1))%md;
            sum = (sum%md + (num1*num)%md)%md;
